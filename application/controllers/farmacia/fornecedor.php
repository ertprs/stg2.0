...
class Fornecedor extends BaseController {
    ...
    function Fornecedor() {
        parent::Controller();
        $this->load->model('farmacia/fornecedor_model', 'fornecedor');
    ...
    function pesquisar($args = array()) {
        ...
        $this->loadView('farmacia/fornecedor-lista', $args);

//            $this->carregarView($data);
    }

    function carregarfornecedor($farmacia_fornecedor_id) {
        $obj_fornecedor = new fornecedor_model($farmacia_fornecedor_id);
        $data['obj'] = $obj_fornecedor;
        $data['tipo'] = $this->fornecedor->listartipo();
        $data['credor_devedor'] = $this->convenio->listarcredordevedor();
        //$this->carregarView($data, 'giah/servidor-form');
        $this->loadView('farmacia/fornecedor-form', $data);
    }

    function excluir($farmacia_fornecedor_id) {
        $valida = $this->fornecedor->excluir($farmacia_fornecedor_id);
        if ($valida == 0) {
        ...
        redirect(base_url() . "farmacia/fornecedor");
    }
