...
class Classe extends BaseController {
    ...
    function Classe() {
        parent::Controller();
        $this->load->model('farmacia/classe_model', 'classe');
    ...
    function pesquisar($args = array()) {
        ...
        $this->loadView('farmacia/classe-lista', $args);

//            $this->carregarView($data);
    }

    function carregarclasse($farmacia_classe_id) {
        $obj_classe = new classe_model($farmacia_classe_id);
        $data['obj'] = $obj_classe;
        $data['tipo'] = $this->classe->listartipo();
        //$this->carregarView($data, 'giah/servidor-form');
        $this->loadView('farmacia/classe-form', $data);
    }

    function excluir($farmacia_classe_id) {
        $valida = $this->classe->excluir($farmacia_classe_id);
        if ($valida == 0) {
        ...
        redirect(base_url() . "farmacia/classe");
    }
