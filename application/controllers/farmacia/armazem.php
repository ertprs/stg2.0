...
class Armazem extends BaseController {
    ...
    function pesquisar($args = array()) {
        ...
        $this->loadView('farmacia/armazem-lista', $args);

//            $this->carregarView($data);
    }

    function carregararmazem($farmacia_armazem_id) {
        $obj_armazem = new armazem_model($farmacia_armazem_id);
        $data['obj'] = $obj_armazem;
        //$this->carregarView($data, 'giah/servidor-form');
        $this->loadView('farmacia/armazem-form', $data);
    }

    function excluir($farmacia_armazem_id) {
        $valida = $this->armazem->excluir($farmacia_armazem_id);
        if ($valida == 0) {
        ...
        redirect(base_url() . "farmacia/armazem");
    }
    ...
    function armazemtransferencia() {
        ...
        $this->loadView('farmacia/transferencia-form', $data);
    }
