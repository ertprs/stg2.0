...
class Cliente extends BaseController {
    ...
    function pesquisar($args = array()) {
        ...
        $this->loadView('farmacia/cliente-lista', $args);

//            $this->carregarView($data);
    }

    function carregarcliente($farmacia_cliente_id) {
        $obj_cliente = new cliente_model($farmacia_cliente_id);
        $data['obj'] = $obj_cliente;
        $data['menu'] = $this->cliente->listarmenu();
        //$this->carregarView($data, 'giah/servidor-form');
        $this->loadView('farmacia/cliente-form', $data);
    }

    function excluir($farmacia_cliente_id) {
        $valida = $this->cliente->excluir($farmacia_cliente_id);
        if ($valida == 0) {
        ...
        redirect(base_url() . "farmacia/cliente");
    }
    ...
    function clientesetor($operador_id) {
        ...
        $this->loadView('farmacia/clientesoperador-form', $data);
    }
