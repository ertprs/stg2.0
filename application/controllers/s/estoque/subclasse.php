...
class Subclasse extends BaseController {
    ...
    function Subclasse() {
        parent::Controller();
        $this->load->model('estoque/subclasse_model', 'sub_classe');
    ...
    function pesquisar($args = array()) {
        ...
        $this->loadView('estoque/subclasse-lista', $args);

//            $this->carregarView($data);
    }

    function carregarsubclasse($estoque_sub_classe_id) {
        $obj_sub_classe = new subclasse_model($estoque_sub_classe_id);
        $data['obj'] = $obj_sub_classe;
        $data['classe'] = $this->sub_classe->listartipo();
        //$this->carregarView($data, 'giah/servidor-form');
        $this->loadView('estoque/subclasse-form', $data);
    }

    function excluir($estoque_sub_classe_id) {
        $valida = $this->sub_classe->excluir($estoque_sub_classe_id);
        if ($valida == 0) {
        ...
        redirect(base_url() . "estoque/subclasse");
    }
