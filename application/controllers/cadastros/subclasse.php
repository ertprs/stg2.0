...
class Subclasse extends BaseController {
    ...
    function Subclasse() {
        parent::Controller();
        $this->load->model('cadastro/subclasse_model', 'sub_classe');
    ...
    function pesquisar($args = array()) {
        ...
        $this->loadView('cadastros/subclasse-lista', $args);

//            $this->carregarView($data);
    }

    function carregarsubclasse($financeiro_sub_classe_id) {
        $obj_sub_classe = new subclasse_model($financeiro_sub_classe_id);
        $data['obj'] = $obj_sub_classe;
        $data['classe'] = $this->sub_classe->listartipo();
        //$this->carregarView($data, 'giah/servidor-form');
        $this->loadView('cadastros/subclasse-form', $data);
    }

    function excluir($financeiro_sub_classe_id) {
        $valida = $this->sub_classe->excluir($financeiro_sub_classe_id);
        if ($valida == 0) {
        ...
        redirect(base_url() . "cadastros/subclasse");
    }
