...
//            $this->carregarView($data);
    ...
    function gravardesconto($convenio_id) {
        ...
        $data['convenioid'] = $convenio_id;
        $mensagem = array('Sucesso ao gravar ajuste', 'success');
    ...
    function excluir($convenio_id) {
        if ($this->convenio->excluir($convenio_id)) {
            $mensagem = array('Sucesso ao excluir a Convenio', 'success');
        } else {
            $mensagem = array('Erro ao excluir a Convenio. Operação cancelada', 'error');
        }
    ...
    function gravar() {
        ...
        if ($convenio_id == "-1") {
            $data['mensagem'] = array('Erro ao gravar Convenio. Opera&ccedil;&atilde;o cancelada', 'error');
        } else {
            $data['mensagem'] = array('Sucesso ao gravar Convenio.', 'success');
        }
    ...
    function gravarcopia() {
        ...
        if ($convenio_id == "-1") {
            $data['mensagem'] = array('Erro ao copiar Convenio. Opera&ccedil;&atilde;o cancelada', 'error');
        } else {
            $data['mensagem'] = array('Sucesso ao copiar Convenio.', 'success');
        }
        $this->session->set_flashdata('message', $data['mensagem']);
        redirect(base_url() . "cadastros/convenio");
    }
