...
class Unidade extends BaseController {
    ...
    function Unidade() {
        parent::Controller();
        $this->load->model('estoque/unidade_model', 'unidade');
    ...
    function pesquisar($args = array()) {
        ...
        $this->loadView('estoque/unidade-lista', $args);

//            $this->carregarView($data);
    }

    function carregarunidade($estoque_unidade_id) {
        $obj_unidade = new unidade_model($estoque_unidade_id);
        $data['obj'] = $obj_unidade;
        //$this->carregarView($data, 'giah/servidor-form');
        $this->loadView('estoque/unidade-form', $data);
    }

    function excluir($estoque_unidade_id) {
        $valida = $this->unidade->excluir($estoque_unidade_id);
        if ($valida == 0) {
        ...
        redirect(base_url() . "estoque/unidade");
    }
