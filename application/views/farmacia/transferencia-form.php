<div class="content"> <!-- Inicio da DIV content -->
    <div id="accordion">
        ...
        <div>
            <form name="form_entrada" id="form_entrada" action="<?= base_url() ?>farmacia/armazem/gravartransferencia/" method="post">
                ...
                <dl class="dl_desconto_lista">
                    ...
                    <dd>
                        <select name="armazementrada" id="armazementrada" class="size4" required="">
                            ...
                            <? foreach ($armazem as $value) : ?>
                                ...
                                if (@$obj->_farmacia_armazem_id == $value->farmacia_armazem_id):echo'selected';
                                endif;
                    ...
                    <dd>
                        <select name="armazem" id="armazem" class="size4" required="">
                            ...
                            <? foreach ($armazem as $value) : ?>
                                <option value="<?= $value->farmacia_armazem_id; ?>"<?
                                if (@$obj->_farmacia_id == $value->farmacia_armazem_id):echo'selected';
                                endif;
                                ?>><?php echo $value->descricao; ?></option>
                                    <? endforeach; ?>
                        </select>
...
//                $('#entrada').hide();
                ...
                $.getJSON('<?= base_url() ?>autocomplete/armazemtransferenciaentradaprodutofarmacia', {produto: $(this).val(), armazem: $("#armazem").val()}, function (j) {
...
//                $('#entrada').hide();
                $('.carregando').show();
//                alert('teste');
                $.getJSON('<?= base_url() ?>autocomplete/armazemtransferenciaentradafarmacia', {produto: $(this).val(), armazem: $("#armazem").val()}, function (j) {
                    ...
                    for (var i = 0; i < j.length; i++) {
                        ...
                        options += '<option value="' + j[i].farmacia_produto_id + '">QTDE: ' + j[i].total + '  Produto:  ' + j[i].descricao + ' Armazem:' + j[i].armazem + '  </option>';
                    }
...
//                $('#entrada').hide();
//                $('.carregando').show();
                $.getJSON('<?= base_url() ?>autocomplete/armazemtransferenciaentradaquantidadefarmacia', {produto: $(this).val(), armazem: $("#armazem").val()}, function (j) {
                    var options = '<option value=""></option>';
                    for (var i = 0; i < j.length; i++) {
                        options += '<option value="' + j[i].farmacia_entrada_id + '">QTDE: ' + j[i].total + '  Produto:  ' + j[i].descricao + ' Armazem:' + j[i].armazem + '  </option>';
                    }
