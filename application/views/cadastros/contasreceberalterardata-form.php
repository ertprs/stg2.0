<div class="content"> <!-- Inicio da DIV content -->
    ...
    <div id="accordion" >
        ...
        <div>
            <form name="form_contasreceber" id="form_contasreceber" action="<?= base_url() ?>cadastros/contasreceber/gravaralterardata" enctype="multipart/form-data" method="post">
            <table>
            <tr>
...
//                                    echo $item->parcela, "ª"; 
                    ...
                    <td class="<?php echo $estilo_linha; ?>">
                        <input type="hidden" name="contasreceber_id[]" id="contasreceber_id<?=$i?>" class="texto04" alt="date" value="<?= @$item->financeiro_contasreceber_id ?>" required=""/>
...
<script type="text/javascript">
    ...
    function mostrarParcelasRepetir() {
        ...
        if(repetir_str > 0 && inicio_str != ''){
            $.getJSON('<?= base_url() ?>cadastros/contasreceber/repetirDiaDePagamento', {repetir: repetir_str, inicio: inicio_str, ajax: true}, function (j) {
                // console.log(j);
                var tr = '';
                $('#tableParcelas').html('');
                for (var c = 0; c < j.length; c++) {
