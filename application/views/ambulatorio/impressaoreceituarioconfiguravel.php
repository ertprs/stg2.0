...
if ($empresapermissoes[0]->desativar_personalizacao_impressao == 'f') { 
    ...
    if (file_exists("./upload/operadortimbrado/" . $laudo['0']->medico_parecer1 . ".png")) {
        ...
        $timbrado = true;
    } elseif (file_exists("./upload/timbradoreceituario/timbrado.png")) {
        $caminho_background = base_url() . 'upload/timbradoreceituario/timbrado.png';
        $timbrado = true;
    } else {
        $timbrado = false;
    }
    ...
    $corpo = str_replace("_municipiodata_",$texto_rodape, $corpo);
    $corpo = str_replace("_endereco_", $laudo[0]->logradouro.", ".$laudo[0]->numero.", ".$laudo[0]->bairro.", ".$laudo[0]->cidade, $corpo);
    $corpo = str_replace("_crm_", $laudo[0]->cbo_ocupacao_id, $corpo);
    $corpo = str_replace("_ocupacao_", $laudo['0']->descricao, $corpo);
