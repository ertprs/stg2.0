...
<div >
    ...
    if (count($receita) == 0) {
        ...
        $receituario_id = $receita[0]->tarefa_receituario_id;
        $medico = $receita[0]->medico_responsavel;
    }
    $operador_id = $this->session->userdata('operador_id');
    ?>

    <div >
        <form name="form_laudo" id="form_laudo" action="<?= base_url() ?>ambulatorio/laudo/gravarreceituariotarefa/<?= $tarefa_medico_id ?>" method="post">
            <div >
                <fieldset>
                    <legend>Dados</legend>
                    <table> 
                        <tr><td width="400px;">Paciente:<?= $obj[0]->paciente ?></td>
                            <td width="400px;">Tarefa: <?= @$obj[0]->nome_tarefa ?></td>
                            <td>Medico responsável: <?= @$obj[0]->medico_responsavel ?></td>
                        </tr>
                        <tr><td>Idade: <?= $teste ?></td>
                            <td>Nascimento:<?= substr(@$obj[0]->nascimento, 8, 2) . "/" . substr(@$obj[0]->nascimento, 5, 2) . "/" . substr(@$obj[0]->nascimento, 0, 4); ?></td>
                ...
                <div>
                    ...
                    <fieldset>
                        ...
                        <div>
                            ...
                            <input type="hidden" id="tarefa_medico_id" name="tarefa_medico_id" value="<?= $tarefa_medico_id ?>"/>
                            ...
                            <tr>
                               ...
                               <!-- <td class="<?php echo $estilo_linha; ?>"><?= $item->procedimento; ?></td> -->
                                ...
                                <td class="<?php echo $estilo_linha; ?>" width="60px;"><div class="bt_link">
                                 <a onclick="javascript:window.open('<?= base_url() ?>ambulatorio/laudo/impressaoreceitatarefa/<?= $item->tarefa_receituario_id; ?>');">Imprimir
                                        </a></div>
                                </td>
                            
                                <td class="<?php echo $estilo_linha; ?>" width="60px;"><div class="bt_link">
                                        <a onclick="repetir(<?=$item->tarefa_receituario_id;?>)">Repetir
                                        </a></div>
                                </td>
                                 <? 
                                if (@$obj[0]->status != 'FINALIZADO'){ 
                                    if(@$obj[0]->medico_parecer1 == $item->operador_id){ ?>
                                    <td class="<?php echo $estilo_linha; ?>" width="60px;"><div class="bt_link">
                                        <a onclick="javascript:window.open('<?= base_url() ?>ambulatorio/laudo/editarcarregarreceituariotarefa/<?= $tarefa_medico_id ?>/<?= $item->tarefa_receituario_id; ?>');">Editar
                                        </a></div>
                                    </td>
                                 <? } else{ ?>
...
<script type="text/javascript">
                                    ...
                                    function repetir(receita_id) {
                                        ...
                                        $.getJSON('<?= base_url() ?>autocomplete/repetirreceituariotarefa', {receita: receita_id, ajax: true}, function (j) {
                                            options = "";
                                            options += j[0].texto;
                                            $('#laudo').val(options);
                                            var ed = tinyMCE.get('laudo');
