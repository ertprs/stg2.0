...
// $AN- variavel, com o accession number( numero do exame), obtida do sistema gestor da clinica;
    $AN = $agenda_exames_id;
    $ipPACS_LAN = $pacs[0]->ip_local; //Ip atribuibdo ao PACS, na LAN do cliente;
                    ...
                    <table> 
                        ...
                        <tr>
                            ...
                            <? if (in_array('prontuario_antigo', $opc_dadospaciente)) { ?>
                                <td colspan="5" style="width: 200px">Prontuário antigo: <?= @$obj->_prontuario_antigo ?></td>
                            <? } ?>
                            ...
                            <div>
                                ...
                                <fieldset>
                                    ...
                                    <div>
                                        ...
                                        <? if(@$empresapermissao[0]->diagnostico_medico == 't'){ ?>
                                            <input type="hidden" name="diagnostico_id" id="diagnostico_id" value="<?=$diagnostico_id?>">
                                            <select id="opcoes" name="opcoes" class="size2" >
                                                        <option value='' >selecione</option>
                                                        <?php foreach ($opcoes as $item) { ?>
                                                            <option value="<?=$item?>" <? //if (@$obj->_opcoes_diagnostico == $item) echo 'selected'; ?>><?=$item?></option>
                                                        <?php } ?>
                                                    </select> 
                                        <br><br>
                                        <label class="carregando">Nivel 1</label>
                                        <select name="nivel1" id="nivel1" class="size2 carregando" data-placeholder="Selecione" tabindex="1">
                                        <option value="">Selecione</option>
                                        </select>

                                        <label class="carregando2">Nivel 2</label>
                                        <select name="nivel2" id="nivel2" class="size2 carregando2" data-placeholder="Selecione" tabindex="1">
                                        <option value="">Selecione</option>
                                        </select>

                                        <label class="carregando2_campolivre">Nivel 2 - Campo Livre</label>
                                        <textarea class="carregando2_campolivre" name="nivel2_campolivre" id="nivel2_campolivre"></textarea>

                                        <label class="carregando3">Nivel 3</label>
                                        <select name="nivel3" id="nivel3" class="size2 carregando3" data-placeholder="Selecione" tabindex="1">
                                        <option value="">Selecione</option>
                                        </select>

                                        
                                        <label class="carregando3_campolivre">Nivel 3 - Campo Livre</label>
                                        <textarea class="carregando3_campolivre" name="nivel3_campolivre" id="nivel3_campolivre"></textarea>
...
<!--                                                      <a onclick="javascript:window.open('<?= base_url() ?>ambulatorio/guia/pesquisarsolicitacaosadt/<?= $paciente_id ?>/<?= @$obj->_convenio_id ?>/<?= @$obj->_medico_parecer1 ?>');" >
                                                                    <font size="-1">Solicitação SADT</font></a>-->
...
//                            $historico = array();
                            ...
                            <script type="text/javascript">
                                    ...
                                    $(function () {
                                        var nivel1_diagnostico = "<?php echo $obj->_nivel1_diagnostico;?>";
                                        var nivel2_diagnostico = "<?php echo $obj->_nivel2_diagnostico;?>";

                                        $('.carregando').hide();
                                        $('.carregando2').hide();
                                        $('.carregando3').hide();
                                        $('.carregando2_campolivre').hide();
                                        $('.carregando3_campolivre').hide();

                                        // // SELECIONADO OPÇÕES
                                        // if ($('#opcoes').val() != '') {
                                        //     $.getJSON('<?= base_url() ?>autocomplete/nivel1diagnostico', {opcoes: $('#opcoes').val(), diagnostico_id: $('#diagnostico_id').val(), ajax: true}, function (j) {
                                        //         options = '<option value=""></option>';
                                        //         for (var c = 0; c < j.length; c++) {
                                        //                 res = j[c].replace($('#opcoes').val() + '-', '');
                                        //                 res2 = res.replace('_', ' ');
                                        //                 options += '<option value="' + res + '" id="selecionar'+res+'" >' + res2 + '</option>';
                                        //         }
                                        //                 $('#nivel1 option').remove();
                                        //                 $('#nivel1').append(options);
                                        //                 $("#nivel1").trigger("chosen:updated");
                                        //                 document.getElementById("selecionar"+nivel1_diagnostico).selected = "true";
                                        //                 $('.carregando').show();
                                        //     });
                                        // } 

                                        // if ($('#nivel1').val() != '') {
                                        //     $.getJSON('<?= base_url() ?>autocomplete/nivel2diagnostico', {nivel1: $('#nivel1').val(), diagnostico_id: $('#diagnostico_id').val(), ajax: true}, function (j) {
                                        //         options = '<option value=""></option>';
                                        //         for (var c = 0; c < j.length; c++) {
                                        //                 res = j[c].replace($('#nivel1').val() + '-', '');
                                        //                 res2 = res.replace('_', ' ');
                                        //                 options += '<option value="' + res + '">' + res2 + '</option>';
                                        //         }

                                        //         if(res2 == 'campolivre'){
                                        //             $('.carregando2_campolivre').show();
                                        //         }else{
                                        //             $('#nivel2 option').remove();
                                        //                 $('#nivel2').append(options);
                                        //                 $("#nivel2").trigger("chosen:updated");
                                        //                 //document.getElementById("selecionarnivel1"+nivel2_diagnostico).selected = "true";
                                        //                 $('.carregando2').show();
                                        //         }
                                        //     });
                                        // }
                                        ...
                                        if ($('#opcoes').val()) {
                                            ...
                                            $('.carregando2_campolivre').hide();
                                            $('.carregando3_campolivre').hide();
                                            $('#nivel2_campolivre').val('');
                                            $('#nivel3_campolivre').val('');
                                            $.getJSON('<?= base_url() ?>autocomplete/nivel1diagnostico', {opcoes: $('#opcoes').val(), diagnostico_id: $('#diagnostico_id').val(), ajax: true}, function (j) {
                                                options = '<option value=""></option>';
                                                for (var c = 0; c < j.length; c++) {
                                                        res = j[c].replace($('#opcoes').val() + '-', '');
                                    ...
                                    $('#nivel2').change(function () {
                                        if ($('#nivel2').val()) {
                                            $.getJSON('<?= base_url() ?>autocomplete/nivel3diagnostico', {nivel2: $('#nivel2').val(), diagnostico_id: $('#diagnostico_id').val(), ajax: true}, function (j) {
                                                options = '<option value=""></option>';
                                                for (var c = 0; c < j.length; c++) {
                                                        res = j[c].replace($('#nivel2').val() + '-', '');
                                                        res2 = res.replace('_', ' ');
                                                        options += '<option value="' + res + '">' + res2 + '</option>';
                                                }

                                                if(res2 == 'campolivre'){
                                                    $('.carregando3_campolivre').show();
                                                }else{
                                                        $('#nivel3 option').remove();
                                                        $('#nivel3').append(options);
                                                        $("#nivel3").trigger("chosen:updated");
                                                        $('.carregando3').show();
                                                }
                                            });
                                        }else{
...
?>
                                                                                        ...
                                                                                        height: 600,
                                                                                                plugins: [
                                                                                                        "advlist save autolink autosave link image lists charmap print preview hr anchor pagebreak",
                                                                                                ...
                                                                                                toolbar2: "cut copy paste | searchreplace | bullist numlist | outdent indent blockquote | undo redo | link unlink pagebreak save anchor image media code | insertdatetime preview | forecolor backcolor | table | removeformat",
...
<? endif; ?>
                                                                                        ...
                                                                                        texto_antigo.replace(texto_adicional_html, "");
                                                                                        texto_antigo.replace(texto_adicional_body, "");
